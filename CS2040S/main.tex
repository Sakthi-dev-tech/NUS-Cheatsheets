...
\end{lstlisting}
    ...
    \begin{itemize}
        \item arr[i][j] gives the weight (1 if unweighted) of the edge from $i$ to $j$ (or 0
              if no edge)
    \end{itemize}
    ...
    \begin{itemize}
        \item Uses a stack to explore the graph as far as possible along each branch before
              backtracking
        \item Time Complexity: $O(n + m)$
        \item Space Complexity: $O(n)$
    \end{itemize}
    ...
    \begin{itemize}
        \item Linear ordering of vertices such that for every directed edge $uv$, vertex $u$
              comes before $v$ in the ordering
        \item Can be performed using DFS
        \item Use linked list to store the topological order (add vertex to front of list
              after visiting all its neighbors)
        \item Time Complexity: $O(n + m)$
        \item Space Complexity: $O(n)$
    \end{itemize}

    \subsection*{SSSP - Relaxation of Nodes}
    \begin{itemize}
        \item Set the source node's distance to $0$, and all other nodes' distances to
              $\infty$.
        \item For each directed edge $u \to v$, if $dist[u] + weight(u, v) < dist[v]$, update
              $dist[v]$ to $dist[u] + weight(u, v)$.
        \item \textbf{Execution Strategy:}
              \begin{itemize}
                  \item \textbf{Bellman-Ford:} Repeat this relaxation for all edges until no updates can be made (maximum $n-1$ iterations).
                  \item \textbf{Dijkstra \& DAG:} Process nodes systematically (greedy or topological order) so edges only need to be relaxed once per visited node.
              \end{itemize}
        \item \textbf{Runtimes} (where $n=|V|$ and $m=|E|$):
              \begin{itemize}
                  \item \textbf{Bellman-Ford:} $O(nm)$
                  \item \textbf{DAG Shortest Path:} $O(n + m)$
                  \item \textbf{Dijkstra's Algorithm:} $O(m \log m)$ using lazy deletion, or $O((n+m)\log n)$ using a standard binary min-heap.
              \end{itemize}
    \end{itemize}

    \begin{minipage}[t]{0.84\linewidth}
        \begin{center}
            \includegraphics[width=0.8\linewidth]{images/djikstra_lazy.png}
        \end{center}
    \end{minipage}

    \subsection*{Union-Find Disjoint Sets (UFDS)}
    Maintains a collection of disjoint sets, allowing for highly efficient union
    and connectivity checks.

    \textbf{Core Arrays:}
    \begin{itemize}
        \item \texttt{parent\_array[x]}: Initially, every element is its own root (\texttt{parent\_array[x] = x}).
        \item \texttt{rank[x]}: Initially, every element's rank is 0.
    \end{itemize}

    \begin{minipage}[t]{0.84\linewidth}
        \begin{center}
            \includegraphics[width=0.8\linewidth]{images/union_by_rank.png}
        \end{center}
    \end{minipage}

    \subsubsection*{Time Complexity}
    \begin{itemize}
        \item \textbf{Worst-Case Runtime:} $O(\log n)$ per operation.
        \item \textbf{Amortized Runtime:} $O(\alpha(n))$ per operation, where $\alpha(n)$ is the Inverse Ackermann function.
        \item \textbf{Practical Implication:} $\alpha(n) \le 4$ for any realistically imaginable input size ($A(4,4) = 2^{2^{65536}} - 3$), meaning operations are effectively $O(1)$. A sequence of $m$ operations takes practically $O(m)$ time.
    \end{itemize}

    \subsection*{Minimum Spanning Trees (MST)}
        \begin{itemize}
            \item \textbf{Assumption:} Edge weights are bounded non-negative integers $w \in [0, c)$ for a constant $c$.
        \end{itemize}

        \subsubsection*{Faster Kruskal's Algorithm}
        \begin{itemize}
            \item \textbf{Idea:} Replace the $O(m \log m)$ comparison-based sort with \textbf{Counting Sort}.
            \item \textbf{Time Complexity:} $O(c + m \alpha(n))$, where $\alpha$ is the inverse Ackermann function.
        \end{itemize}

        \subsubsection*{Faster Prim's Algorithm}
        \begin{itemize}
            \item \textbf{Idea:} Replace the $O(\log n)$ Priority Queue with an array of $c$ queues. To find the minimum edge, scan the array from index $0$ to $c-1$ to find the first non-empty queue.
            \item \textbf{Time Complexity:} $O(c(n + m))$
        \end{itemize}

        \textbf{Pseudocode:}
        \begin{verbatim}
            in_tree = array of size n, initialized to false
            fake_pq = array of c empty queues

            in_tree[s] = true
            for (n, weight) in get_neighbours(s): 
                fake_pq[weight].push(n)

            while fake_pq has a non-empty queue:
                curr_node = pop from the first non-empty queue in fake_pq
                if in_tree[curr_node]: 
                    continue
                    
                in_tree[curr_node] = true
                for (n, weight) in get_neighbours(curr_node): 
                    fake_pq[weight].push(n)
        \end{verbatim}
