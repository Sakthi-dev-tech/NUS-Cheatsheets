...
\begin{itemize}
    ...
    \item \textit{Application:} $\lim_{n \to \infty} n \sin(\frac{k}{n}) = k$.
\end{itemize}
...
For $x=f(t), y=g(t)$:
\[ \frac{dy}{dx} = \frac{g'(t)}{f'(t)} \quad \text{and} \quad \frac{d^2y}{dx^2} = \frac{\frac{d}{dt}(y')}{dx/dt} \]
...
\subsection*{Tangent Half-Angle ($z=\tan \frac{\theta}{2}$)}
...
\begin{itemize}
    ...
    \[L = \int_c^d \sqrt{(\frac{dx}{dt})^2 + (\frac{dy}{dt})^2} dt\]
\end{itemize}
...
\[
    S_n = \sum_{i=1}^{n} a_i \cdot r^{i-1} = a_1(\frac{1-r^n}{1-r})
\]
\begin{itemize}
    \item Diverges if $|r| \geq 1$
    \item Converges to $\frac{a}{1-r}$ if $|r| \le 1$
\end{itemize}
...
\subsection*{Test for Convergence}
    ...
    \end{itemize}
    $$
        \int_{1}^{\infty}f(x)dx \textbf{ converges}
    $$
    ...
    $$
        L = \lim_{n\xrightarrow{}\infty} |\frac{a_{n+1}}{a_n}|
    $$
    \begin{itemize}
    ...
    $$
        L = \lim_{n \xrightarrow{} \infty} \sqrt[n]{|a_n|}
    $$
    \begin{itemize}
    ...
    $$
    \begin{itemize}
        \item If $b_n$ is decreasing $(b_{n+1} \leq b_n)$ and $\lim_{n\xrightarrow{}\infty}b_n = 0$, then the series converges.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize} 
            ...
            \item $R = \frac{1}{L} \xrightarrow{}$ L is the limit from Ratio or Root test applied to coeffs
        \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Length:} $||\mathbf{a}|| = \sqrt{a_1^2 + a_2^2 + a_3^2}$
    \item \textbf{Unit Vector:} $\mathbf{u} = \frac{\mathbf{a}}{||\mathbf{a}||}$
\end{itemize}

\subsection*{The Dot Product}
\begin{itemize}
    \item $\mathbf{a} \cdot \mathbf{b} = a_1 b_1 + a_2 b_2 + a_3 b_3 = ||\mathbf{a}|| ||\mathbf{b}|| \cos \theta$
    \item $\mathbf{a} \perp \mathbf{b} \iff \mathbf{a} \cdot \mathbf{b} = 0$
\end{itemize}

\subsection*{Projections}
\begin{itemize}
    \item \textbf{Scalar Projection (comp):} $$\text{comp}_\mathbf{a} \mathbf{b} = \frac{\mathbf{a} \cdot \mathbf{b}}{||\mathbf{a}||}$$
    \item \textbf{Vector Projection (proj):} $$\text{proj}_\mathbf{a} \mathbf{b} = \left(\frac{\mathbf{a} \cdot \mathbf{b}}{||\mathbf{a}||^2}\right) \mathbf{a}$$
...
\begin{itemize}
    \item \textbf{Definition:} 
            ...
            \end{vmatrix}
            \]
    \item \textbf{Magnitude:} $||\mathbf{a} \times \mathbf{b}|| = ||\mathbf{a}|| ||\mathbf{b}|| \sin \theta$
    \item $\mathbf{a} \times \mathbf{b}$ is orthogonal to both $\mathbf{a}$ and $\mathbf{b}$.
    \item \textbf{Point Q to Line PR Dist:} $d = \frac{||\vec{PQ} \times \vec{PR}||}{||\vec{PR}||}$
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Plane (Scalar):} $ax + by + cz + d = 0$ (with $\mathbf{n} = \langle a,b,c \rangle$)
\end{itemize}
...
\section*{Ch 8: Functions of Several Variables}
    ...
    \begin{itemize}
        ...
        \item General Case, where u is a differentiable function of n variables $x_1$,...,$x_n$ and each $x_j$ is a differentiable function of m variables $t_1, ...,t_m$. Then u is a function of $t_1,...,t_m$:
